...
- **Job executor + workflow (đính kèm worksheet JTBD / ảnh sơ đồ):**
  - *Đối tượng thực thi (Job executor):* Học viên ngành AI / Kỹ thuật đang tự học một bài mới (đặc biệt là bài thực hành/tính toán công nghệ như Tokenization, Chi phí API LLM), có thói quen làm thử bài tập trước khi đọc tài liệu lý thuyết dài dòng.
  ...
  - *Workflow mong muốn:* Đọc đề bài → Tự làm trước lý thuyết → Nếu sai, AI phân tích đúng loại ngộ nhận, đưa 1 gợi ý súc tích kèm mã section tài liệu (`§1.2`) → Học viên tự sửa bài → Giải thích bản chất vì sao sai → Mở khóa toàn bộ lý thuyết chuẩn.
...
- **Evidence (chuẩn A và/hoặc B — log đầy đủ trong repo):**
  - **Số liệu mining / kết quả khảo sát (n = 20 học viên):**
  ...
  - **≥5 quote/ví dụ nguyên văn + nguồn:**
    1. *"Em hay nhảy vào làm bài luôn cho đỡ buồn ngủ, nhưng sai một cái là tắc tị, lật lại slide mấy chục trang không biết xem ở đâu."* — (Học viên K4, khảo sát thực tế)
    2. *"Nhiều khi nhìn lời giải thấy hiểu liền, nhưng tuần sau gặp lại dạng tương tự vẫn sai đúng chỗ đó vì có tự nghĩ ra đâu."* — (Học viên K3, khảo sát thực tế)
    3. *"Em tính ra số token tiếng Việt bằng số từ, thấy sai đáp án nhưng không hiểu sao sai, tưởng mình cộng trừ nhầm chứ không biết là do tiếng Việt nó khác."* — (Học viên lớp AI20k, phỏng vấn đào sâu)
    4. *"Nếu có ai chỉ nhẹ cho em một manh mối gợi ý và bảo em đọc đúng mục nào thì em tự sửa được ngay, không cần đưa cả bài giải ra đâu."* — (Học viên K4, khảo sát thực tế)
    5. *"Lúc sửa bài xong em muốn giải thích lại xem mình hiểu đúng chưa, chứ nhiều khi sửa bừa theo gợi ý trúng đáp số nhưng bụng vẫn mơ hồ."* — (Học viên K4, phỏng vấn đào sâu)
...
## §4. Thiết kế
- **Lát cắt MỘT CÂU (1 user · 1 việc · 1 quyết định AI · 1 kết quả):** Một học viên làm bài tập ước tính token/chi phí trước khi học lý thuyết; nếu làm sai, AI chẩn đoán cụ thể loại lỗi, đưa một gợi ý tư duy ngắn kèm mã section tài liệu liên quan (`§1.2`), rồi thẩm định xem học viên có tự sửa đúng và giải thích được nguyên nhân gốc rễ hay không.
...
| Tình huống (Kịch bản) | Lớp chỗ khó | Biểu hiện cụ thể (Học viên nhập) | Hành vi mong muốn (AI Behavior) | Nguyên tắc áp dụng |
|---|---|---|---|---|
| **Coi 1 từ tiếng Việt = 1 token** | Lớp 1: Khái niệm sai lệch | `100 câu x 20 từ = 2,000 từ = 2,000 token` | Chỉ ra ngộ nhận về định nghĩa token; gợi ý xem lại cơ chế tách từ; dẫn nguồn `§1.2`. Không lộ đáp án. | HAX G2 (Minh bạch lỗi), No-spoiler |
| **Áp dụng nhầm hệ số tiếng Anh** | Lớp 1: Khái niệm sai lệch | `2,000 từ x 1.3 = 2,600 token` | Cảnh báo tiếng Anh và tiếng Việt có bộ ký tự khác nhau; gợi ý tra cứu `§1.2`. | HAX G2 (Trích nguồn chính xác) |
| **Sai mẫu số đơn vị quy đổi** | Lớp 2: Lỗi công thức & đơn vị | `(5,000 / 1,000) * 0.5 = 2.5 USD` | Nhắc nhở đơn giá tính trên 1 triệu token (1M), không phải 1 ngàn token; dẫn `§2.1`. | HAX G2, Socratic Hint |
| **Quên nhân hệ số số lượng câu** | Lớp 2: Lỗi công thức & đơn vị | `50 / 1,000,000 * 0.5 = $0.000025` | Nhắc học viên kiểm tra phạm vi đề bài là 100 câu hay 1 câu; dẫn `§2.1`. | HAX G2, Socratic Hint |
| **Nhập chuỗi ký tự rác hoặc cảm tính** | Lớp 3: Mơ hồ / Thiếu căn cứ | `asdfghjk` hoặc *"Chi phí rẻ lắm"* | Không đoán mò; báo chưa đủ căn cứ đánh giá; yêu cầu học viên trình bày cách tính. | HAX G10 (Thu hẹp phạm vi khi nghi ngờ) |
| **Đầu hàng hoặc đoán số không bước giải** | Lớp 3: Mơ hồ / Thiếu căn cứ | `Em chịu` hoặc `Chắc là $1` | Khuyên học viên đọc tài liệu gợi ý hoặc thử nhẩm bước đầu tiên; kiên quyết không giải hộ. | HAX G10, Socratic Method |
| **Giải thích sâu sắc nguyên nhân sai** | Lớp 4: Kiểm tra hiểu bản chất | `Do ban đầu em tưởng 1 từ là 1 token, nhưng tiếng Việt có dấu thanh UTF-8 nên BPE tách thành 2-3 tokens` | Chúc mừng đã nắm vững bản chất; xác nhận hoàn thành bài tập và mở khóa toàn bộ lý thuyết. | Track D2 Core (Đánh giá thấu đáo) |
| **Chép vẹt / đổ lỗi công cụ** | Lớp 4: Kiểm tra hiểu bản chất | `Tại lúc nãy AI bảo sai nên em sửa lại thế` | Không duyệt; yêu cầu học viên giải thích lại lý do thực sự vì sao tiếng Việt lại tốn nhiều token hơn. | Track D2 Core (Chống chép vẹt) |

> **Tự kiểm:** *Kịch bản làm nhóm sợ nhất khi demo là gì?*  
> 👉 **Kịch bản học viên nhập vừa có số đúng vừa có câu mơ hồ/hỏi vặn lại AI** (ví dụ: `5000 tokens nhưng em đoán bừa thế thôi`). Nếu AI vội vàng khen đúng và mở luôn đáp án thì vi phạm nghiêm trọng nguyên tắc HAX G10 và làm hỏng mục tiêu sư phạm. Nhóm đã xử lý bằng guardrail 2 tầng: kiểm tra căn cứ trước, chỉ khi có lập luận/bước tính hợp lệ mới chuyển tiếp.
...
- **Failure/không căn cứ (①):** Bài làm là chuỗi ký tự ngẫu nhiên hoặc thể hiện bỏ cuộc (`asdf`, `em chịu`) → AI thông báo chưa đủ dữ liệu để hỗ trợ, hướng dẫn học viên đọc đoạn tài liệu nền tảng thay vì đoán mò.
- **Correction (user sửa):** Sau gợi ý lần 1, học viên sửa lại đúng kết quả → Kích hoạt ngay bước hỏi giải thích nguyên nhân sai lầm ban đầu.
...
- **Quality bar (chốt từ hạn chốt spec của khoá, giữ nguyên sau đó): "Đạt khi ≥ 75% qua bộ, và đồng thời thoả mãn 3 điều kiện cứng":**
  > 1. Tỷ lệ ca vượt qua bộ test tự động 22 cases đạt **≥ 75%** (Mức thực tế CP3: 68.2%, mục tiêu tinh chỉnh prompt: ≥ 80%).
  > 2. **100% ca gợi ý không để lộ đáp án cuối** (No-spoiler Guardrail).
...
- **TỰ KHAI PHẦN CHƯA XONG (Theo quy định CP4 Sổ tay):**
...
- **Phân công có tên: spec / evidence / prompt / code / demo:**
  - **Ngô Tuấn Tùng (Lead):** Chịu trách nhiệm chính về Spec, Canvas, cấu trúc thư mục repo, điều phối tiến độ và thiết kế Slide pitch CP5/CP6.
...
- **Willing users (≥2 tên) + kế hoạch vòng validation *(bonus, nếu làm)*:**
  ...
  - *Kế hoạch:* Cho từng bạn thao tác độc lập, ghi hình phiên thử nghiệm tại `validation/`, phỏng vấn nhanh theo Mom Test và ghi nhận phản hồi để tinh chỉnh giao diện trước CP5.
- **Multi-prototype (nếu làm): trục khác biệt của ≥2 phương án + lý do chọn:**
  - *Phương án A:* Đưa gợi ý một lần duy nhất kèm tài liệu, nếu sai tiếp thì hiện đáp án ngay.
  - *Phương án B (Phương án chọn):* Vòng lặp tối đa 2 lần gợi ý tăng dần độ chi tiết + bắt buộc giải thích bản chất trước khi mở lý thuyết toàn diện.
  - *Lý do chọn:* Khảo sát cho thấy 65% học viên học theo lối thử và sai; phương án B giảm 100% tình trạng xem đáp án thụ động và kích thích tư duy phản tư sâu sắc.
