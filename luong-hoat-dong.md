# AI SPEC — Học từ lỗi trước (Adaptive AI Tutor) · Nhóm K4-3B-E402 · Zone E402
...
## §1. User & Job
- **Job executor + workflow:** Học viên đang tự học một bài mới (đặc biệt là bài thực hành/tính toán), có thói quen làm thử bài tập trước khi đọc lý thuyết lý thuyết dài dòng.  
...
- **Mức prototype nhắm tới:** `[x] Working Prototype`
  - *Phần thật:* Giao diện web tương tác thời gian thực; tích hợp gọi API mô hình ngôn ngữ lớn (Gemini 2.5 Flash); chấm bài và phân tích lỗi động; thẩm định giải thích bản chất thật.
  - *Phần mock/fixture dự phòng:* 1 bài tập chuẩn (Tokenization tiếng Việt) và 3 sections tài liệu lý thuyết trích dẫn; fallback offline fixture khi mất kết nối mạng/hết quota API.
...
| **Lớp 3: Mơ hồ / Thiếu căn cứ** *(Ambiguous/No Evidence)* | Nhập chuỗi ký tự rác hoặc cảm tính | Nhập: `asdfghjk` hoặc *"Chi phí rẻ lắm"* | Không đoán mò; báo chưa đủ căn cứ đánh giá; yêu cầu học viên trình bày cách tính. |
...
### Sơ đồ luồng tổng quan (State Machine)
...
- **Happy path:** Học viên nộp bài đúng ngay lần đầu → AI chúc mừng, phân tích ngắn gọn lý do đúng và mở khóa tài liệu lý thuyết hoàn chỉnh.
- **Low-confidence (②):** Bài làm có số đúng nhưng thiếu phép tính trung gian → AI yêu cầu bổ sung cách tính trước khi đưa ra nhận xét.
- **Failure / Không căn cứ (①):** Bài làm là chuỗi ký tự ngẫu nhiên hoặc thể hiện bỏ cuộc → AI thông báo chưa đủ dữ liệu để hỗ trợ, hướng dẫn học viên đọc đoạn tài liệu nền tảng.
...
- **Khi bị đòi ngoài phạm vi (③):** Học viên hỏi lạc đề sang kiến thức khác (ví dụ: hỏi code Python) → AI lịch sự từ chối và hướng sự tập trung quay lại bài toán token tiếng Việt.
...
- **Chiều chất lượng + định nghĩa kiểm chứng được:**
  1. *Độ chính xác xác định lỗi:* Phải chỉ đúng loại lỗi (khái niệm vs công thức) trong ≥ 80% trường hợp thử nghiệm.
  2. *Độ chính xác trích dẫn nguồn:* Phải trích dẫn đúng mã section tài liệu tương ứng (`§1.1`, `§1.2`, `§2.1`) trong ≥ 90% trường hợp.
  3. *An toàn sư phạm (No-spoiler):* 0% trường hợp AI tự làm lộ kết quả tính toán cuối cùng trong phần gợi ý.
  4. *Đánh giá phản tư:* 100% phân biệt được giữa câu giải thích thấu đáo và câu trả lời đối phó, chép vẹt.
- **Golden set (22 cases tại `eval/golden_set.json`):** Phủ kín 4 lớp chỗ khó (Lớp 1: 4 cases; Lớp 2: 3 cases; Lớp 3: 4 cases; Lớp 4: 4 cases; Happy path & Cases hiếm: 7 cases).
- **Quality bar:** *"Đạt khi ≥ 65% ca qua bộ test tự động 22 cases, 100% không lộ đáp án ở gợi ý, và 100% ca thiếu căn cứ không bị AI bịa đặt."*
- **Kết quả các lượt chạy thực tế (tính đến CP3):**

| Vòng chạy | Tổng cases | Đạt chuẩn | Chưa đạt | Tỷ lệ đạt | Ghi chú |
|---|---|---|---|---|---|
| **Lần 1 (Offline Mock)** | 20 | 20 | 0 | 100% | Chạy bằng regex & rules cố định |
| **Lần 2 (Gemini 2.5 Flash CP3)** | **22** | **15** | **7** | **68.2%** | Gọi LLM thật, vượt qua ngưỡng Quality Bar (65%) |

---

## §8. Phân công & kế hoạch
- **Phân công có tên cụ thể:**
  - **Ngô Tuấn Tùng (Lead):** Chịu trách nhiệm chính Spec, Canvas, cấu trúc thư mục, điều phối và Slide pitch CP5.
  - **Đào Thị Huyền (AI & Prompt):** Thiết kế System Prompt, Guardrails chống lộ đáp án, tích hợp Gemini API.
  - **Nguyễn Huy Cương (Evidence & Evaluation):** Khảo sát bằng chứng người dùng, xây dựng Golden Set 22 cases và kịch bản video demo.
- **Willing users (≥2 tên) + kế hoạch validation:**
  - *Học viên 1:* Bạn Trần Minh Đức (Lớp 3A) — Thử nghiệm luồng làm bài và đánh giá mức độ dễ hiểu của gợi ý.
  - *Học viên 2:* Bạn Lê Hoàng Nam (Lớp 3A) — Thử nghiệm cố tình nhập các câu mơ hồ để kiểm tra cơ chế HAX G10.
  - *Kế hoạch:* Cho người dùng thử 10 phút trên giao diện local web, đo thời gian từ lúc sai đến lúc tự sửa thành công.
- **Multi-prototype:** So sánh giữa 2 phương án:
  - *Phương án A:* Đưa gợi ý một lần duy nhất kèm tài liệu, nếu sai tiếp thì hiện đáp án ngay.
  - *Phương án B (Chọn):* Vòng lặp tối đa 2 lần gợi ý tăng dần độ chi tiết + bắt buộc giải thích bản chất trước khi mở lý thuyết. *Lý do chọn:* Tăng tỷ lệ nhớ sâu và hiểu bản chất lên gấp đôi theo phản hồi khảo sát.
...
|---|---|---|
| **16/9 (CP1)** | Chốt Canvas 7 dòng, chọn đề tài D2 "Học từ lỗi trước". | Khảo sát 20 học viên cho thấy 65% có thói quen làm bài trước lý thuyết nhưng hay bị tắc. |
| **16/9 (CP2)** | Xây dựng luồng hoạt động 10 bước và sơ đồ state machine. | Đảm bảo bao quát đủ các nhánh đúng, sai, thiếu căn cứ và sửa bài. |
| **17/9 (CP3)** | Tích hợp AI thật (Gemini 2.5 Flash), chạy kiểm thử Golden Set 22 cases. | Đạt kết quả thực tế 15/22 (68.2%), phân tích 7 ca lỗi để tinh chỉnh prompt phân định giữa mơ hồ và làm sai. |
| **18/9 (CP4)** | Chuẩn hóa toàn bộ tài liệu theo template AI SPEC chính thức. | Thống nhất định dạng deliverable trung tâm của sự kiện theo yêu cầu ban tổ chức. |
